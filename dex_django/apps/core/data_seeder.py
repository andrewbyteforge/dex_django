# APP: backend
# FILE: backend/app/core/data_seeder.py
...
class CopyTradingSeeder:
    ...
    # NOTE: These are example wallet addresses. You would need to find actual 
    # wallet addresses of successful traders through:
    # 1. DEX analytics platforms (DexScreener, DEXTools)
    # 2. On-chain analysis (Nansen, Arkham Intelligence) 
    # 3. Public wallet disclosure by traders
    # 4. Transaction analysis of successful trades
    ...
    SEED_TRADERS = [
        ...
        },
        {
            ...
            "copy_buy_only": False,
            "copy_sell_only": False,
            "status": WalletStatus.PAUSED,  # Start paused for review
    ...
    async def seed_copy_trading_data(self, force_reseed: bool = False) -> Dict[str, Any]:
        ...
        try:
            async with get_db() as session:
                ...
                for trader_data in self.SEED_TRADERS:
                    try:
                        ...
                        if trader_data["status"] == WalletStatus.ACTIVE:
                            await copy_trading_coordinator.add_tracked_wallet(
                                ...
                                nickname=trader_data["trader_name"],
                                copy_settings={
                                    "copy_mode": trader_data["copy_mode"].value,
                                    "copy_percentage": float(trader_data.get("copy_percentage", 3.0)),
                                    "fixed_amount_usd": float(trader_data.get("fixed_amount_usd", 100)),
                ...
                return {
                    ...
                    "errors": errors,
                    "total_attempted": len(self.SEED_TRADERS)
                }
        ...
        try:
            async with get_db() as session:
                ...
                return {
                    "status": "success",
                    "wallet_stats": stats,
                    "seed_traders_available": len(self.SEED_TRADERS),
